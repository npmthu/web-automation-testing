...
4. Move the generated steps into `tests/login-example.spec.js`. (This file is a teaching example for this guide; the team's full FR-02 suite — lockout, edge cases, and a security/escalation check — lives separately in `tests/fr02-login-A.spec.js`, `fr02-login-B.spec.js`, and `fr02-login-C.spec.js`, WAT-06.)
...
| A second `playwright.config.js` was found inside `frontend-web/`, separate from the root project | This is by design, not a mistake: the root project (`tests/`) drives the login flows, while `frontend-web/` has its own config (chromium-only, auto-starts the dev server) for the cart/checkout/AI-track suites — see the folder structure note at the end of this section | Confirmed both configs are intentional and kept both; only fixed the actual bug, which was `login.spec.js` having been saved outside `testDir` (row above) |
...
**A note on project structure, since it looks unusual at first glance:** this repository intentionally contains **two separate Playwright projects**, not a duplicate. `playwright.config.js` at the repo root drives `tests/` (the login flows, FR-02 — chromium + firefox + webkit, backend/frontend started manually as in Section 2). `frontend-web/playwright.config.js` drives `frontend-web/tests/` (cart, checkout, and the AI-track suites — chromium only, auto-starts its own dev server via `webServer`). If you `cd frontend-web` before running a command, you are in the second project; running from the repo root, you are in the first. Both configs should stay in the repository — do not delete either one when cloning fresh.
